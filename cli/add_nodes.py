...
def update_node_publishers(node, current_pubs, pub_list):
    ...
    for pub in pub_list:
        if pub not in current_pubs:
            ...
            if err:
                print "\t\tERROR: failed to add publisher {0} for node {1}"                     .format(pub, node, server)
        else:
            # publisher already exists
            print "\t\tINFO: publisher {0} already exists for node {1}"                 .format(pub, node, server)
...
if __name__ == '__main__':
    ...
    for server in servers:
        ...
        if server not in host_dict.keys():
            ...
            print("WARNING: host {0} not found in ~/.netrc. Skipping..."                 .format(server))
            continue
        else:
